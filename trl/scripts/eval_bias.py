...
def hack_package(package_path, replace=False):
    ...
    (Path(package_path) / "chat_template.json").unlink(missing_ok=True)
    # this could be generated by deepspeed somehow
    (Path(package_path) / "model.safetensors").unlink(missing_ok=True)
